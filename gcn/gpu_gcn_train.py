...
from gcn.graph import *
from gcn.gcn_input_utils import *
import time
...
def train_model(model, graph, y, A, args):
    wait = 0
    preds = None
    best_val_loss = 99999
    y_train, y_val, y_test, idx_train, idx_val, idx_test, train_mask = get_splits(y)
    for epoch in range(1, NB_EPOCH + 1):
        t = time.time()
        model.fit(graph, y_train, sample_weight=train_mask, batch_size=graph[0].shape[0], epochs=1, shuffle=False, verbose=0)
        preds = model.predict(graph, batch_size=graph[0].shape[0])
        train_val_loss, train_val_acc = evaluate_preds(preds, [y_train, y_val], [idx_train, idx_val])
...
class GraphModel:
    def __init__(self, feature_matrix, adjacency_matrix, labels):
        self.feature_matrix = feature_matrix
        self.adjacency_matrix = adjacency_matrix
        self.labels = labels
        self.graph = create_input_graph(self.adjacency_matrix, self.feature_matrix)
        self.model = self.define_model()

    def define_model(self, support=1):
        G = [Input(shape=(None, None), batch_shape=(None, None), sparse=True)]
